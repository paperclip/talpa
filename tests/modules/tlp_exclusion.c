...
#define TALPA_SUBSYS "exclusiontest"
...
int talpa_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long parm)
{
    ...
    exclusion->i_IInterceptFilter.enable(exclusion);
        ...
        case TALPA_TEST_FILEINFO:
            ...
            if ( !ret )
            {
                ...
                if ( fi )
                {
                    ...
                    fi->mFSType = tf.fstype;
                    exclusion->i_IInterceptFilter.examineFile(exclusion, &erep->i_IEvaluationReport, &pers->i_IPersonality, &fi->i_IFileInfo, NULL);
                    fi->mFSType = oldfs;
                    fi->delete(fi);
        ...
        case TALPA_TEST_FILESYSTEMINFO:
            ...
            if ( !ret )
            {
                ...
                if ( fsi )
                {
                    ...
                    fsi->mType = tfs.type;
                    exclusion->i_IInterceptFilter.examineFilesystem(exclusion, &erep->i_IEvaluationReport, &pers->i_IPersonality, &fsi->i_IFilesystemInfo);
                    fsi->mType = oldfs;
                    fsi->delete(fsi);
                    ret = erep->i_IEvaluationReport.recommendedAction(erep);
                }
...
static int __init talpa_test_init(void)
{
    ...
    exclusion = newFilesystemExclusionProcessor();
    ...
    if ( !erep )
    {
        ...
        mConfig->delete(mConfig);
        exclusion->delete(exclusion);
        mSystemRoot->delete(mSystemRoot);
        return 1;
    }

    ret = mConfig->i_IConfigurator.attach(mConfig, ECG_InterceptFilter, &exclusion->i_IConfigurable);
    ...
    if ( ret )
    {
        ...
        mConfig->i_IConfigurator.detach(mConfig, &exclusion->i_IConfigurable);
        erep->delete(erep);
        exclusion->delete(exclusion);
        mConfig->delete(mConfig);
        mSystemRoot->delete(mSystemRoot);
